...
''' This function is used during inference when selecting tokens '''
...
class PredictorLG(nn.Module):
    ...
    ''' The predictiton module take the current policy and the hidden states as input '''
    ...
    def forward(self, x, policy):
        ...
        ## Combine both local and global features to obtain the local-global embeddings
        
        x = torch.cat([local_x, global_x.expand(B, N, C//2)], dim=-1)

        ## We use the local-global embeddings to predict the probabilities to drop/keep the tokens
...
'''The encoder has been slightly modified compared to that of the original model in order to be able to calculate the policy based on which tokens are dropped.
'''
    ...
    def forward(
        ...
        for layer in self.layers:
            ...
            if c in [7,14,21]: ## Layer in which the policy is updated
               ...
               if self.training and (dropout_probability < self.config.layerdrop): # Skip the layer
                   ...
                   if getattr(self.config, "gradient_checkpointing", False) and self.training:
                       ...
                       ## Now we generate the current decision passing the pred_score (probabilities to drop/keep the tokens) to the softmax
                       ...
                       ## Reshape the policy in order to use it as attention mask
                       ...
                       ## Dring the inference we take the top n score and drop the other token
                       score = pred_score[:,:,0]
...
class MyWav2Vec2Model(Wav2Vec2PreTrainedModel):
    def __init__(self, config):
        ...
        if config.do_stable_layer_norm:
            ...
            self.encoder = MyWav2Vec2Encoder(config)
    ...
    def forward(
        ...
        if attention_mask is not None:
            ...
            '''print("--------------")
            print("Attention Mask pre encoder")
            #print(attention_mask.shape)
            print(attention_mask)
            print(" ")'''
